...
export function WaveformEditor({ slot, onSave, onClose }) {
  ...
  const handleSave = useCallback(() => {
    onSave(startPercent * duration, endPercent >= 0.99 ? null : endPercent * duration, volume)
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50" onClick={onClose}>
      ...
      >
        ...
        <p className="text-zinc-400 text-sm mb-1">{slot.name ?? slot.fileName}</p>
        ...
        <div className="mt-4 flex items-center gap-3">
          ...
          <input type="number" min={minLength} max={duration} step="0.05"
            value={windowLength.toFixed(2)} onChange={e => handleLengthChange(Number(e.target.value))}
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button onClick={onClose} className="px-4 py-2 bg-zinc-700 hover:bg-zinc-600 text-white rounded-lg transition">
            Cancel
          </button>
          <button onClick={handleSave} className="px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg transition">
            Save
          </button>
        </div>
      </div>
